...
# Install the tidyverse package if you haven't done so previously
# install.packages('tidyverse') # run this one time to install tidyvere package, which enables you to use ggplot functions
...
psize_per_plate = plq_size_filt %>% group_by(group, strain, plate) %>% summarise(mean_diam = mean(plaque_diam_mm), mean_area = mean(plaque_area))
...
ggplot(data=psize_per_group, aes(x=strain, y=mean_diam, col=strain)) + geom_boxplot(width=0.35,outlier.shape = NA)+ geom_jitter(position=position_jitter(0.1),size=2) + ylab("Average Plaque Area (mm^2)")
...
ggplot(data=plq_num_filt, aes(x=strain,y=num_plaques, col=strain)) + geom_boxplot(outlier.shape = NA) + geom_jitter(position=position_jitter(0.2),size=2) + ylab('Number of Plaques') + ylab("Strain")

ggplot(data=pnum_avg_across_plates, aes(x=strain,y=mean_plqs, col=strain)) + geom_boxplot(outlier.shape = NA) + geom_jitter(position=position_jitter(0.2),size=2) 

ggplot(data=pnum_avg_across_plates, aes(x=strain, y=mean_plqs, col=strain)) + geom_boxplot(width=0.2,outlier.shape = NA) + geom_jitter(aes(fill=strain), position=position_jitter(0.2),size=2) + ylab("Mean Number of Plaques") + xlab("Strain") 
...
ggplot(data=all_data, aes(x=mean_plqs, y=mean_area)) + geom_point() + xlab('Number of Plaques') + ylab('Plaque Area') + geom_smooth(method='lm')

cor.test(all_data$mean_plqs, all_data$mean_area)
...
ggplot(pnum_avg_across_plates, aes(x = strain, y = mean_plqs, group = student_1)) +
  geom_line(alpha = 0.7, color = "gray") +
  geom_point(size = 2) +
  theme_minimal() +
  labs(title = "Reaction Norm Plot",
       x = "Condition",
       y = "Response") +
  theme(
